...
<body>
    ...
    <script>
        ...
        function matchesAnyDistrict(district, activeDistrictFilters) {{
            if (activeDistrictFilters.length === 0) {{
                return true; // No filters active, show all
            }}

            return activeDistrictFilters.some(function(filter) {{
                return district === filter.value;
            }});
        }}

        function style(feature) {{
            var score = feature.properties.opportunity_index;
            var district = tractDataLookup[feature.properties.CensusTract]?.district;
            ...
            var matchesDistrict = matchesAnyDistrict(district, activeDistrictFilters);
